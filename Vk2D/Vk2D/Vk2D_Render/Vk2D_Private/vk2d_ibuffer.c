...
vk2d_ibuffer* vk2d_create_ibuffer(vk2d_gpu* gpu, vk2d_device* device, vk2d_command* command, i32 listSize, void* vertices)
{
    ...
    if (result)
    {
        ...
        create_buffer(gpu, device, &result->buffer, &result->buffer_memory, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
vk2d_ibuffer* vk2d_create_ibuffer_empty(vk2d_gpu* gpu, vk2d_device* device, vk2d_command* command, i32 listSize)
{
    ...
    if (result)
    {
        ...
        create_buffer(gpu, device, &result->buffer, &result->buffer_memory, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
